...
# Project2_PartA.asm - This program takes a the input of a user string and counts how many times the characters K,N,I,G,H,T,S
# are found and outputs this result as both integer values and a histogram chart
#
# Registers used:
# $t0 - saves the input string in memory
# $t1 - is set to 1 if uppercase K or lower case k character is found in the string
# $t2 - is set to 1 if uppercase N or lower case n character is found in the string
# $t3 - is set to 1 if uppercase I or lower case i character is found in the string
# $t4 - is set to 1 if uppercase G or lower case g character is found in the string
# $t5 - is set to 1 if uppercase H or lower case h character is found in the string
# $t6 - is set to 1 if uppercase T or lower case t character is found in the string
# $t7 - is set to 1 if uppercase S or lower case s character is found in the string
...
lb $k1, 0($t0) # loads the data at the current memory location into a register to be later used for comparison
beqz $k1, exit # if reg $k1 = NULL (comparator register) exit the loop we have reached the end of the string


# checks if any of the letters in KNIGHTS upper case or lower case are found in current byte address if so branch to the respective location
seq $t1, $k1, 75 	#K
beq $t1, 1, K
seq $t1, $k1, 107	#k
beq $t1, 1, K
...
j loop # if no exit condition is met jumps back to the beginning of loop

exit:
# end of loop

# the number of K, N, I, G, H, T, S are found in the input string are outputted using syscall instrucitons
li $v0, 4
la $a0, k
syscall
li $v0, 1
